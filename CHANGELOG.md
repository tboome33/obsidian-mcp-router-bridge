...
## [0.8.0] — 2026-08-09 — vault conformance check on open: detection only, default OFF

> *Written 2026-08-31, from the shipped code and its tests — this entry was
> missed at release time. Facts below are read out of `src/conformance-core.mjs`
> / `src/conformance.ts`, not reconstructed from memory.*

A router-managed vault carries generated navigation under `wiki/`: a root `index.md`, one `index.md` per ancestor directory of every content page, and a newest-first `log.md`. They are projections, regenerated by the router — and **nothing runs when a vault is merely opened in Obsidian**, so a vault whose projections were never built, or that drifted after folders were created by hand, looks perfectly fine until something far away misbehaves. This closes exactly that observation gap, and nothing more: on layout-ready, once, the bridge lists the vault's markdown, computes which projections *should* exist, checks each present one for the router's marker, and raises a single Notice if anything is missing, unmarked, or unreadable.

### What it deliberately does NOT do

- **It never generates.** There is exactly one implementation of the projection generator and it lives in the router (`okf-projections.mjs`, shared with the OKF exporter). Porting it here — another language, another repo, another release train — would create a second implementation that diverges the first time either side is fixed, with both writing different bytes into the same files. The whole module is free of writes: no `adapter.write`, and no string that could become file content. The Notice names the router as the thing that repairs, because this plugin does not and will not.
- **It never writes on open.** Opening a vault must not mutate it. The only outputs are a Notice and an in-memory report the settings tab renders.
- **It says nothing about a vault it does not manage.** The first draft keyed applicability on "is there markdown under `wiki/`" — wrong in the direction that produces false accusations, since plenty of vaults have a hand-made `wiki` folder the router never touched. What identifies a managed vault is the provisioner's private scaffold (`wiki-meta/catalog.md`, or `wiki-meta/index.md` on a pre-0.58.0 vault). No scaffold, no opinion — and on such a vault the check costs a Set membership probe over the two candidate paths and zero reads.

### Default OFF, and that is not timidity

The bridge auto-updates through BRAT; a behaviour that appeared by itself in every existing vault after an upgrade would be an unannounced UI change — same reasoning as the folder-hiding switch (0.6.0). Opt in per vault; the settings tab carries a re-check button and a status line that can say what a Notice cannot ("conformant", "not managed", "the check itself failed").

### The honest limits, stated in the code rather than papered over

- **The marker and scaffold names are copied constants, deliberately.** The two repos ship and auto-update independently, so there is no import to share. The tests pin each constant to its literal *within this repository* — which catches a careless local edit and **cannot** catch router-side drift: the assertion never observes the router (measured: changing the marker on the router alone leaves this suite fully green). A cross-repo pin was considered and rejected as false-reliable — it would pass only on a dev machine with both repos side by side and be absent-or-broken in CI and in the shipped bundle. If the router ever changes a constant, the symptom depends on WHICH one: scaffold-name drift silences the check (the vault stops looking managed), while marker drift makes it *noisy in the wrong direction* — every regenerated file starts looking hand-written, so the Notice accuses a healthy vault. (The module header claims the symptom is silence in both cases; tracing the code shows the marker case actually reports `unmarked` — the header's own claim is the one that drifted.) Either way, the module header names the coupling so a router change prompts a matching edit.
- **A failed check is not a verdict.** A check that threw is recorded as `failed` — a state distinct from both "conformant" and "not managed" — logged to the console once, and never turned into a Notice. An expected projection that exists but cannot be read lands in `unreadable[]` and makes the vault `incomplete`: unverified is never reported as fine.
- **Every expected projection is marker-checked, not just the two at the root.** An unmarked `wiki/a/index.md` is a hand-written file the router classifies as a conflict and refuses to overwrite; reporting the vault conformant while the router reports a conflict would be the two halves of one feature disagreeing about the same file.
- **An empty managed vault is a checkable state, not "not a wiki"**: it still expects the root index and the log, which is exactly what the provisioner writes into a newborn vault.

Marker reads are bounded (8 in flight) so a vault expecting dozens of directory indexes does not turn a "cheap check" into a visible hitch on the UI thread at open. The Notice caps each category's path list (missing / unmarked / unreadable) at 3 with `+N` beyond, and is bilingual FR/EN.

### Tests

`tests/conformance-core.test.mjs` — 43 cases: the four silent/loud branches of `runOpenCheck`, the empty-managed-vault case, search-index presence (presence ONLY — never version or freshness), bounded reads, the pinned local literals with their honestly-stated scope, path classification (a user's `Index.md` stays content), ancestor-index expansion, Notice and status-line wording.

## [0.7.0] — 2026-08-01 — `PUT /vault-cas/<path>`: the write that refuses to clobber

> *Written 2026-08-31, from the shipped code and its tests — this entry was
> missed at release time. Facts below are read out of
> `src/handlers/vault-cas-core.mjs` / `vault-cas.ts`, not reconstructed from
> memory. Includes the same-day review pass (`55f2a53`): request→core mapping
> extracted pure, plus race/bytes/vector pins.*

The router's C1: optimistic concurrency for vault writes. Two parallel sessions editing the same note silently clobbered each other — the second write won and the first vanished without a trace. `PUT /vault-cas/<vault-relative-path>` is the fix's bridge half: the caller sends the full new content plus an `If-Match-Content-Sha256` header carrying the hash its edit was based on, and the write is applied **only if the file still hashes to that value at the serialized check** (the scope of that "still" is stated honestly below). Otherwise a `409` with machine-readable `reason` (`content-changed` with both hashes, or `target-missing`) and the caller re-reads instead of overwriting.

### The contract's sharpest edge: this handler NEVER returns 404

A raw 404 here means one thing only — *the route is absent* (older bridge, or bridge disabled) — which is what lets the router degrade cleanly to its own GET-compare-then-PUT tier. Any request this route can see but not service answers 400 (bad path/encoding/body/precondition) or 403 (traversal), never 404; the router also treats a 413/415 — which would come from Local REST API's own middleware, not from this handler — as "atomic tier unusable, fall back". Only a genuine 409 conflict (and auth/network/5xx) is a hard error. The 0.9.0 route later borrowed the never-404 rule verbatim.

### Atomicity — honest scope, written into the docblock

The read→decide→write critical section runs inside a module-level mutex (`withCasLock`), and because Obsidian's vault runs on a single JS event loop, that makes the section indivisible **against other `/vault-cas` writes**. It is *not* indivisible against a writer that does not go through this route: a plain core `PUT /vault` (the router's default, non-ifMatch path), a save from the open Obsidian editor, or a LiveSync apply can still land between our read and our write. C1 prevents CAS-vs-CAS clobbering; full clobber-prevention requires every writer to use ifMatch. That is inherent to optimistic concurrency, and the entry says so rather than implying a guarantee the mechanism cannot make.

### One hash, two runtimes, pinned so they cannot drift

The hash MUST match the router's `content-hash.mjs`: SHA-256 over the UTF-8 bytes, lowercase hex, **no normalization** (CRLF and LF hash differently, on purpose). The router uses `node:crypto`; the bridge runs in the Electron renderer with Node builtins externalized, so it uses Web Crypto — and both suites pin the same known vectors (`sha256("hello")`, the empty string, an astral surrogate pair, a large body) so a drift on any of the pinned behaviours cannot go unnoticed — a finite vector set proves no more than that, but it covers every divergence mode the two runtimes are known to have.

One deliberate exception to "no normalization": **a single leading BOM (U+FEFF) is stripped before hashing**, identically on both sides. The bridge reads via `adapter.read()`, which keeps a BOM; the router's `get_file` reads via `GET /vault` → `res.text()`, which strips it. Without the strip, a BOM-prefixed file would hash differently on the two sides and every atomic write to it would 409 **forever**. A non-leading U+FEFF is not touched.

### Deliberately not done, and the failure modes each choice closes

- **No "expected-absent" mode.** "Create only if new" is already served by Local REST API's own `Apply-If-Content-Preexists` header (the router's ifNew); ifMatch is strictly "replace iff content still equals this hash", which is only meaningful for a file that exists.
- **A file that vanishes — or becomes unreadable — between `exists()` and `read()`** (concurrent delete/rename, a lock, a mid-sync file) is treated as absent → a clean `409 target-missing` instead of an opaque 500, preserving the caller's re-read-and-retry path. The 409's wording ("no longer exists") can therefore be imprecise for a merely-unreadable file; the safety property is what matters: nothing was written, and a spurious conflict only costs a re-read.
- **The traversal guard errs strict** — this is a WRITE route, where a false positive refuses one write and a false negative could escape the vault. Backslashes and slash-runs are collapsed, then absolute paths, `..` segments and drive letters are rejected.
- **A rejecting task cannot poison the mutex** — later CAS writes still run.
- **Registered via `addRoute`, not `addPublicRoute`**: the Bearer check applies, same trust surface as the core `PUT /vault` it guards.

### Tests

`tests/vault-cas-core.test.mjs` — 40 cases, everything injected so the whole read→decide→write path runs against an in-memory adapter under `node --test`: the pinned cross-repo hash vectors, CRLF≠LF, the BOM regression (on-disk BOM matching the BOM-free hash the router sent), `bytesWritten` counting UTF-8 bytes rather than JS string length, mutex serialization and poison-resistance, the vanish-between-exists-and-read race, and the race that motivates the feature — two CAS writes racing on the same stale token, **exactly one wins**.
